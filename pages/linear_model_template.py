...
def show_model_page(title):
    ...
    with col1:
        ...
        noise_factor = st.slider("Epsilon (Error))", min_value=0.0, max_value=10.0, value=2.0, step=0.1)
    ...
    with col3:
        ...
        x = np.linspace(-5, 10, n_samples).reshape(-1, 1)
        y = intercept + slope * x.flatten() + epsilon
    ...
    with col4:
        ...
        if title.lower() == "linear regression":

            X = sm.add_constant(x)
            model = sm.OLS(y, X).fit()
            
            fig, ax = plt.subplots(figsize=(8, 6))
            sns.scatterplot(x=x.flatten(), y=y, alpha=0.5, label='Data', color='#ff9900', ax=ax)
            sns.lineplot(x=x.flatten(), y=model.predict(X), color='#8a2be2', linewidth=2, label='Fitted Model', ax=ax)
